...
--2.1 Выбрать из таблицы film значения полей id_periodical, у которых поле name_of_periodical равно NULL
...
--4.2 Выбрать из таблицы courier номер телефона, наибольший по длине
SELECT MAX(phone) FROM courier;
...
--6.1 Выбрать из таблицы периодических изданий имя и общий заработок актёра и сгруппировать всё по имени
SELECT name_of_periodical, Sum(price) FROM periodical GROUP BY name_of_periodical;

--6.2 Выбрать из таблицы периодических изданий название издания и его общую стоимость, при условии что стоимость журнала больше 0 рублей, сгруппировать по названию издания
SELECT name_of_periodical, Sum(price) FROM periodical WHERE (price > 0) GROUP BY name_of_periodical;


--6.3 Выбрать из таблицы периодических изданий поля имя и общую стоимость изданий, при условии что общая стоимость журналов больше 50 рублей
